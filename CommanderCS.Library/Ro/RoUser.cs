...
namespace CommanderCS.Library.Ro
{
    ...
    public class RoUser
    {
        ...
        public void SetGroupCompleteData(List<int> list)
        {
            ...
            completeRewardGroupList.Sort((a, b) => regulation.groupInfoDtbl.Find((data) => int.Parse(data.groupIdx) == b).typeIndex.CompareTo(regulation.groupInfoDtbl.Find((data) => int.Parse(data.groupIdx) == a).typeIndex));
        }
        ...
        public List<string> GetBuffIdxList()
        {
            ...
            foreach (KeyValuePair<EWorldDuelBuff, EWorldDuelBuffEffect> data in activeBuff)
            {
                ...
                StrongestBuffBattleDataRow strongestBuffBattleDataRow = regulation.strongestBuffBattleDtbl.Find((row) => row.buffTarget == data.Key && row.buffEffectType == data.Value && row.buffLevel == level);
