...
namespace CommanderCS.Library.Ro
{
    ...
    public class RoTroop
    {
        ...
        public EBranch branch => RoCommander.Create(commanderId, 1, 1, 1, 0, 0, 0, [])?.branch ?? EBranch.Undefined;
        ...
        {
            CommanderDataRow commander = _reg.commanderDtbl.Find((row) => row.id == id);
            UnitDataRow unitDataRow = _reg.unitDtbl.Find((row) => row.key == commander.unitId);
            List<RoTroop> list = [];
        ...
        public static List<RoTroop> CreateEventBoss(EventRaidData data)
        {
            ...
            EnemyUnitDataRow enemyUnitDataRow = _reg.enemyUnitDtbl.Find((row) => row.id == data.enemy);
            List<RoTroop> list = [];
        ...
        {
            List<EnemyCommanderDataRow> list = _reg.enemyCommanderDtbl.FindAll((row) => row.id == id);
            List<EnemyUnitDataRow> list2 = _reg.enemyUnitDtbl.FindAll((row) => row.id == id);
            int wave = 1;
            List<RoTroop> list3 = [];
            while (list3.Count < list.Count)
            {
                EnemyCommanderDataRow enemyCommanderDataRow = list.Find((row) => row.wave == wave);
                List<EnemyUnitDataRow> list4 = list2.FindAll((row) => row.wave == wave);
        ...
        public Troop ToBattleTroop()
        {
            ...
            for (int i = 0; i < slots.Length; i++)
            {
                ...
                Troop.Slot slot2 = new()
                {
                    id = slot.unitId,
                    health = slot.health,
                    damagedHealth = 0,
                    level = slot.unitLevel,
                    exp = slot.exp,
                    rank = slot.unitRank,
                    cid = slot.commanderId,
                    cls = slot.unitCls,
                    costume = slot.unitCostume,
