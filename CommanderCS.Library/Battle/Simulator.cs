using CommanderCS.Library.Battle.Internal;
    ...
    public class Simulator
    {
        ...
        private void _InitUnitWorldDuelSkillData(Unit unit, Troop.Slot slot)
        {
            ...
            switch (unitDataRow.job)
            {
                case EJob.Attack:
                case EJob.Attack_x:
                    unit._addAtk += (int)((unitDataRow.attackDamage + unit._addAtk) * (long)dictionary[EWorldDuelBuff.att] / 10000);
                    if (unit._addAtk < 0)
                    {
                        unit._addAtk = 0;
                    }
                ...
                case EJob.Defense:
                case EJob.Defense_x:
                    unit._addDef += (int)((unitDataRow.defense + unit._addDef) * (long)dictionary[EWorldDuelBuff.def] / 10000);
                    if (unit._addDef < 0)
                    {
                        unit._addDef = 0;
                    }
                ...
                case EJob.Support:
                case EJob.Support_x:
                    unit._addAtk += (int)((unitDataRow.attackDamage + unit._addAtk) * (long)dictionary[EWorldDuelBuff.sup] / 10000);
                    if (unit._addAtk < 0)
                    {
                        unit._addAtk = 0;
                    }
        ...
        private void _InitUnitFavorData(Unit unit, Troop.Slot slot)
        {
            ...
            List<FavorDataRow> list = regulation.favorDtbl.FindAll((row) => row.cid == int.Parse(unit._cid) && row.step <= unit.favorRewardStep);
            if (list is null)
            {
                return;
            }
        ...
        private void _InitUnitGroupBuffData(Unit unit, Troop.Slot slot)
        {
            ...
            {
                bool flag = false;
                GroupInfoDataRow groupInfoDataRow = regulation.groupInfoDtbl.Find((data) => data.groupIdx == groupBuffs[gi] && data.rewardType >= ERewardType.GroupEff_1 && data.rewardType <= ERewardType.GroupEff_8);
                ...
                {
                    GroupMemberDataRow groupMemberDataRow = regulation.groupMemberDtbl.Find((row) => row.gidx == groupBuffs[gi] && row.memberType == 1 && row.memberIdx == unit.cid);
        ...
        {
            UnitDataRow unitDataRow = regulation.unitDtbl[unit.dri];
            unit._maxHealth = (int)((unitDataRow.maxHealth + unit._addHp) * (100L + unit.maxHealthBonus) / 100);
        ...
        private void _InitUnitSkillData(Unit unit, Troop.Slot slot)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Troop.Slot.Skill slot2 = slot.skills.Find((x) => x.id == skillDrk);
                int count = unit.skills.Count;
                if (slot.weaponItem is not null && slot.weaponItem.ContainsKey(count))
                {
                    WeaponDataRow weaponDataRow = regulation.weaponDtbl[slot.weaponItem[count].id];
                    if (!_CheckWeaponData(weaponDataRow, Regulation.Regulation.ParseWeaponSkillType(count)))
                    {
                        throw new ArgumentNullException("Fail CheckWeaponData!! " + weaponDataRow.idx);
                    }
            ...
            if (slot.weaponItem is not null && slot.weaponItem.ContainsKey(4))
            {
                ...
                if (!_CheckWeaponData(weaponDataRow2, Regulation.Regulation.ParseWeaponSkillType(4)))
                {
