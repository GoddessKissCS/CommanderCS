...
using BattleSimulator.Battle.Internal;
...
namespace BattleSimulator.Battle
{
	...
	public class Simulator
	{
		...
		public static Simulator Create(Regulation rg, BattleData bd)
		{
			...
			{
                initState._dualData = new()
                {
                    _playerName = bd.attacker.nickname,
                    _playerLevel = bd.attacker.level,
                    _playerGuildName = bd.attacker.guildName,
                    _enemyName = bd.defender.nickname,
                    _enemyLevel = bd.defender.level,
                    _enemyRank = bd.defender.duelRanking,
                    _enemyGuildName = bd.defender.guildName,
                    _enemyUno = bd.defender.uno
                };
                if (bd.defender.guildSkillList != null)
				{
					List<GuildSkillState> list7 = new();
				...
				{
                    initState._dualData._worldDuelData = new()
                    {
                        _playerWorld = bd.attacker.world,
                        _playerBuffs = bd.attacker.GetBuffIdxList(),
                        _enemyWorld = bd.defender.world,
                        _enemyBuffs = bd.defender.GetBuffIdxList()
                    };
                }
			}
		...
		public static Simulator Create(Regulation rg, Record record)
		{
			...
			simulator.record._option = Option.Default;
		...
		{
			Shared.Battle.Random random = new Shared.Battle.Random(initState.randomSeed);
		...
		{
			Shared.Battle.Random random = new Shared.Battle.Random(curr.randomSeed);
			Frame frame = new Frame();
